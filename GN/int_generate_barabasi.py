...
from arguments_barabasi import Arguments
...
class Int_Generating_Data:
	def __init__(self, path, num_nodes, edges_to_attach, num_flows):
		self.path = path
		self.num_nodes = num_nodes
		self.edges_to_attach = edges_to_attach
	...
	def INT_Barabasi(self, num_nodes, edges_to_attach) :
		path = os.path.join(self.path, "Int_Network_" + str(self.num_nodes))
		G = nx.barabasi_albert_graph(num_nodes, edges_to_attach)
		for i in G.edges:
			G[i[0]][i[1]]["weight"] = random.randint(5,10)
		H = nx.DiGraph(G)
		nx.write_weighted_edgelist(G, path + ".txt", comments='#',delimiter=' ',encoding='utf-8')
		nx.write_weighted_edgelist(H, path + ".csv", delimiter=',',encoding='utf-8')
		#nx.write_edgelist(H, path + ".csv", delimiter=',',data=False,encoding='utf-8')
		#nx.write_edgelist(G, path + ".txt", comments='#',delimiter=' ',data=False,encoding='utf-8')
...
if __name__ == "__main__":
	...
	data = Int_Generating_Data(path = arg.instance, num_nodes = arg.num_nodes, edges_to_attach = arg.edges_to_attach, num_flows = arg.num_flows)
